...
function tryEncodeSync(baseInput, targetInput, options) {
  ...
  // The third argument may be a method string ('bytediff') or an options object
  // ({ method, strict, minRatio }). Normalize to an object.
  if (typeof options === 'string') options = { method: options };
  options = options || {};
  var minRatio = (typeof options.minRatio === 'number') ? options.minRatio : 1.0;
  var method = options.method || 'auto';   // 'auto' | 'bytediff' | 'lcs' | 'raw'
  var strict = options.strict === true;     // when forcing a method, skip the RAW safety net
  ...
  // Identical / empty-base shortcuts apply only in auto mode; an explicit
  // method request is honored verbatim so the caller gets exactly what they asked for.
  if (method === 'auto' && bytesEqual(base, target)) {
    ...
    // soft force: still fall back to RAW if the forced delta is bigger than the
    // full target (cheap length check, no extra algorithm run). `strict` opts out.
    if (!strict && payload.length >= target.length) { tag = M_RAW; payload = target; }
  } else {
    // ── auto: try the real methods, pick the smallest ──
...
// Schedules `work()` on a microtask and delivers its result (or error) to the
// Node-style callback `(err, result)`. The callback is ALWAYS invoked
// asynchronously and exactly once — even today, while the work is synchronous.
// This locks the contract so that adding async WASM init later changes nothing
// for callers. (Computing inside the microtask also routes thrown input errors
// through the callback rather than throwing synchronously.)
...
// Pull an optional middle argument (method string or options object) out of the
// (…, [options], callback) tail so callers can omit it.
...
// encode(base, target, [optionsOrMethod], callback) -> callback(err, Uint8Array)
...
// tryEncode(base, target, [optionsOrMethod], callback) -> callback(err, result)
function tryEncode(base, target, opt, cb) {
  var a = splitOptsCb(opt, cb);
  runAsync(a.cb, function () { return tryEncodeSync(base, target, a.options); });
}

// decode(base, delta, callback) -> callback(err, Uint8Array)
function decode(base, delta, cb) {
  runAsync(cb, function () { return decodeSync(base, delta); });
}

// encodeString(base, target, [optionsOrMethod], callback) -> callback(err, Uint8Array)
function encodeString(base, target, opt, cb) {
  var a = splitOptsCb(opt, cb);
  runAsync(a.cb, function () { return tryEncodeSync(utf8Encode(base), utf8Encode(target), a.options).delta; });
}

// decodeString(base, delta, callback) -> callback(err, string)
function decodeString(base, delta, cb) {
  runAsync(cb, function () { return utf8Decode(decodeSync(utf8Encode(base), delta)); });
}

// inspect(base, target, callback) -> callback(err, { method, size, raw, ratio, candidates })
